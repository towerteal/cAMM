...
contract IntegralsVolatile {
    ...
    function _calcPriceCurveArea(
        ...
        // int256 casting is safe. If overflows, it returns negative. 
        // lnWad fails on negative numbers. If the vault balance is high, this is unlikely.
    ...
    function _calcPriceCurveLimit(
        ...
        return FixedPointMathLib.mulWadDown(
            B,
            FixedPointMathLib.WAD - uint256(FixedPointMathLib.expWad(-int256(U / W)))   
            // int256 casting is initially not safe. If overflow, the equation becomes: 1 - exp(U/W) => exp(U/W) > 1.
            // In this case, Solidity's built-in safe math protection catches the overflow.
        );
    }
    ...
    function _calcPriceCurveLimitShare(
        ...
        // int256 casting is initially not safe. If overflow, the equation becomes: exp(U/W). 
        // In this case, when subtracted from 1 (later), 
        // Solidity's built-in safe math protection catches the overflow since exp(U/W) > 1.
